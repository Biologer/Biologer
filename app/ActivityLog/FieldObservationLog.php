...
class FieldObservationLog
{
     ...
     * @param  array  $oldData
     * @param  array|null  $photoSync
     * @return array
     */
    protected function getChangedData(FieldObservation $fieldObservation, array $oldData, $photoSync)
    {
        $excluded = ['taxon_id', 'mgrs10k', 'time', 'observed_by_id', 'identified_by_id', 'original_identification'];
        $changed = array_merge(
        ...
        foreach ($oldData as $key => $value) {
            if ('time' === $key && $this->timeIsChanged($fieldObservation, $value)) {
                ...
                if ('taxon_suggestion' === $key) {
                    ...
                    $data['data_license'] = [
                        'value' => $value,
                        'label' => ($license = License::findById($value)) ? 'licenses.'.$license->name : null,
                    ];
