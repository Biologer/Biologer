...
class AdminTaxaExport extends BaseExport
{
    /**
    ...
    public static function columnData()
    {
        ...
        return collect(array_keys(Taxon::RANKS))->map(function ($rank) {
            ...
            [
                'label' => trans('labels.exports.stage_names'),
                'value' => 'stage_names',
            ],
            [
                'label' => trans('labels.exports.stage_ids'),
                'value' => 'stage_ids',
            ],
        ])->concat($locales->map(function ($locale, $localeCode) {
            $group = trans('labels.exports.group_name');
            $localeTranslation = trans('languages.'.$locale['name']);

            return [
                'label' => "{$group} - {$localeTranslation}",
                'value' => 'group_'.Str::snake($localeCode),
            ];
        }))->concat([
            [
